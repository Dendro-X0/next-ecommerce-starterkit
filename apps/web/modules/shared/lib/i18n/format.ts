...
export function formatCurrency(locale: Locale, amount: number, currency?: string): string {
  ...
  return new Intl.NumberFormat(locale, { style: "currency", currency: cur, currencyDisplay: "symbol" }).format(amount)
}

export function formatDate(locale: Locale, date: Date | number | string, opts: Intl.DateTimeFormatOptions = {}): string {
