...
async function update(id: string, patch: UpdateProductInput): Promise<ProductDTO | null> {
  ...
  if (Array.isArray(patch.media)) (fields as any).media = patch.media as any
  if (typeof patch.kind === "string") (fields as any).kind = patch.kind
  if (typeof patch.shippingRequired === "boolean")
    (fields as any).shippingRequired = patch.shippingRequired
  if (typeof patch.weightGrams === "number") (fields as any).weightGrams = patch.weightGrams
  if (typeof patch.digitalVersion === "string")
    (fields as any).digitalVersion = patch.digitalVersion
...
async function latestCreatedAt(): Promise<string | undefined> {
  ...
  return v ? ((v as any)?.toISOString?.() ?? String(v)) : undefined
}
